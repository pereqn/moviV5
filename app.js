// MOVI v7 — persistent state + theme modes
...
let userSettings = loadLS('movi_settings', {
  theme:'auto', callApp:'phone', mapApp:'google', city:''
});
...
function renderMonthChips(){
  ...
  [-1,0,1].forEach(off=>{
    const dt = new Date(base); dt.setMonth(base.getMonth()+off);
    const label = dt.toLocaleDateString('ru-RU', {month:'2-digit'});
    const chip = document.createElement('button');
    chip.className='chip'+(off===0?' active':'');
    chip.textContent = label;
    chip.addEventListener('click', ()=>{
      currentDate = addMonthsISO(currentDate, off);
      renderMonthChips(); renderDateChips(); renderDay(); renderWeekStats(); renderDayMonthStats(); renderProfileStats();
    });
...
function renderDateChips(){
  ...
  [-1,0,1].forEach((off)=>{
    ...
    chip.addEventListener('click', ()=>{ currentDate = iso; renderDateChips(); renderDay(); renderWeekStats(); renderDayMonthStats(); renderProfileStats(); });
...
function onTouchEnd(e){
  ...
  if(Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)){
    ...
    renderDateChips(); renderDay(true, dx<0 ? 'left':'right'); renderWeekStats(); renderDayMonthStats(); renderProfileStats();
  }
  touchX=touchY=null;
}
  ...
  Object.keys(orders).forEach(key=>{
    ...
    if(kd.getFullYear()===y && kd.getMonth()===m){
      (orders[key]||[]).forEach(o=>{
        monthTotal++;
        if(o.status==='done'){ monthDone++; monthRevenue += (parseInt(o.price||0,10)||0); } else { monthActive++; }
      });
    }
  });
...
$$(".navbtn").forEach((btn,idx)=>{
  btn.addEventListener('click', ()=>{
    ...
    if(map[idx]){
      ...
      if(map[idx]==='settings') syncSettingsUI();
    }
  });
});
$("#profileBtn").addEventListener('click', ()=>{ showView('profile'); renderProfileStats(); syncProfileUI(); });
...
function buildFullAddress(o){
  ...
  if(o.address && !street){ return o.address; } // fallback legacy
  return city + [street, house].filter(Boolean).join(' ') + apt;
}
function digitsForURL(phone){ return (phone||'').replace(/\D/g,'').replace(/^8/,'7'); }
function makeCallUrl(phone){
  const digits = digitsForURL(phone);
  if(userSettings.callApp==='whatsapp') return `https://wa.me/${digits}`;
  if(userSettings.callApp==='telegram') return `tg://resolve?phone=${digits}`;
  return `tel:+${digits}`;
}
...
function cardHTML(item, i, dateISO, conflict=false){
  ...
  return `<div class="card" data-index="${i}" data-date="${dateISO}">
    ...
    <div class="expand">
      ...
      <div class="quick">
        <a class="icbtn" href="${makeCallUrl(item.phone)}" title="Позвонить" aria-label="Позвонить">${svgPhone}</a>
...
function setStatus(date, index, status){
  ...
  persistAll();
  renderDay(); renderDateChips(); renderWeekStats(); renderDayMonthStats(); renderProfileStats();
  ...
  list.innerHTML = requests.map(r=>{
    ...
    return `<div class="card" data-id="${r.id}">
      ...
      <div class="expand">
        ...
        <div class="quick">
          <a class="icbtn" href="${makeCallUrl(r.phone)}" title="Позвонить" aria-label="Позвонить">${svgPhone}</a>
...
function acceptRequest(id){
  ...
  renderDateChips(); renderWeekStats(); renderDayMonthStats(); renderProfileStats();
...
$("#goToDate").addEventListener('click', ()=>{
  ...
  renderMonthChips(); renderDateChips(); renderDay(true,'left'); renderWeekStats(); renderDayMonthStats(); renderProfileStats();
...
document.addEventListener('keydown', (e)=>{
  if(e.key==='ArrowLeft'){ currentDate = addDaysISO(currentDate,-1); renderDateChips(); renderDay(true,'right'); renderWeekStats(); renderDayMonthStats(); renderProfileStats(); }
  if(e.key==='ArrowRight'){ currentDate = addDaysISO(currentDate,1); renderDateChips(); renderDay(true,'left'); renderWeekStats(); renderDayMonthStats(); renderProfileStats(); }
});
